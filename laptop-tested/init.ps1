...
Start-Process -FilePath $PhpCgi -ArgumentList "-c `"$phpDir\php.ini`" -b 127.0.0.1:9000" -WorkingDirectory $phpDir -WindowStyle Hidden
...
if %ERRORLEVEL% NEQ 0 ( echo [!] Failed to download WordPress. & exit /b 1 )
powershell -Command "Expand-Archive -Path '%SITE_DIR%\wp.zip' -DestinationPath '%SITE_DIR%\wp_tmp' -Force"
if %ERRORLEVEL% NEQ 0 ( echo [!] Failed to extract WordPress archive. & exit /b 1 )
xcopy /E /Q "%SITE_DIR%\wp_tmp\wordpress\*" "%PUBLIC_DIR%\" >NUL
if %ERRORLEVEL% GEQ 4 ( echo [!] Failed to copy WordPress files. & exit /b 1 )
...
if %ERRORLEVEL% NEQ 0 ( echo [!] Failed to create Nginx config. & exit /b 1 )
